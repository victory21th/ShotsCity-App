...
- (void)setUpLabel
{
	...
	_colorAccount = [[UIColor alloc] initWithWhite:70.0/255.0 alpha:1.0];
	_colorUser = [[UIColor alloc] initWithWhite:70.0/255.0 alpha:1.0];
    ...
    [self setFontUser:[UIFont boldSystemFontOfSize:13]];
    [self setTextColor:[UIColor blackColor]];
    [self setColorAccount:[UIColor blackColor]];
    [self setColorHashtag:[UIColor blackColor]];
    [self setColorUser:[UIColor blackColor]];
...
- (void)drawTextInRect:(CGRect)rect
{
    ...
    NSRegularExpression *regexForbiddenUser = [NSRegularExpression regularExpressionWithPattern:@"([^A-Z0-9a-z(é|ë|ê|è|à|â|ä|á|ù|ü|û|ú|ì|ï|î|í)._-]+)" options:NSRegularExpressionCaseInsensitive error:&error];
        ...
        for (NSString *wordArray in words)
        {
            ...
            do
            {
                ...
                if (![wordCharacters isEqualToString:@""])
                {
                    ...
                    {
                        if ([wordCharacters hasPrefix:@"|"]) {
                            [[wordCharacters substringFromIndex:1] drawAtPoint:drawPoint withFont:self.fontUser];
                        }else if ([wordCharacters hasPrefix:@"@"]){
                            [[wordCharacters substringFromIndex:1] drawAtPoint:drawPoint withFont:self.font];
                        }else
                ...
                if (![postCharacters isEqualToString:@""])
                {
                    ...
                    else
                    {
                        ...
                        else if ([lastPrefix isEqualToString:@"|"] && [regexForbiddenUser firstMatchInString:postCharacters options:0 range:NSMakeRange(0, [postCharacters length])])
                        {
                            NSTextCheckingResult *matchFor = [regexForbiddenUser firstMatchInString:postCharacters options:0 range:NSMakeRange(0, [postCharacters length])];
...
- (CGSize)getFitSize{
    //CGSize sizeWord = [@"A" sizeWithFont:self.font];
    [sizeLines removeAllObjects];
...
/*    if (!match) {
    ...
    sizeWord = [@" " sizeWithFont:self.font];
    CGSize resultSize = CGSizeMake(self.frame.size.width, sizeWord.height * sizeLines.count);
    [sizeLines removeAllObjects];
    return resultSize;
...
+ (CGFloat) getFitHeightWithText:(NSString*)tweetText boundWidth:(CGFloat)boundWidth font:(UIFont*)tFont color:(UIColor*)tColor{
    STTweetLabel *tempLabel = [[STTweetLabel alloc] initWithFrame:CGRectMake(0, 0, boundWidth, 10)];
    
    [tempLabel setFont:tFont];
    [tempLabel setTextColor:tColor];
    [tempLabel setText:tweetText];
    
    [tempLabel setFont:tFont];
    [tempLabel setFontUser:tFont];
    [tempLabel setTextColor:[UIColor colorWithRed:.3 green:.3 blue:.3 alpha:1]];
    [tempLabel setColorAccount:[UIColor blackColor]];
    [tempLabel setColorHashtag:[UIColor blackColor]];
    [tempLabel setColorUser:[UIColor blackColor]];
    
    
    CGFloat result = 0;
    result = [tempLabel getFitSize].height;
