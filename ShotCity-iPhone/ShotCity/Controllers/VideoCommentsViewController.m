...
@interface VideoCommentsViewController ()

@end

@implementation VideoCommentsViewController

@synthesize shotData, isEditing;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (isReloading) {
        if (isNewUpdate ) {
            ...
            return [VideoCommentCell getCellHeight:[dataList objectAtIndex:indexPath.row - 1]];
        }else if (isNewUpdate == FALSE){
            if (indexPath.row == dataList.count) {
                return 30;
            }
            return [VideoCommentCell getCellHeight:[dataList objectAtIndex:indexPath.row]];
        }
    }
    return [VideoCommentCell getCellHeight:[dataList objectAtIndex:indexPath.row]];
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    VideoCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    ...
    if(!data) {
        ...
        if (jsonData) {
            ...
            if (![aStr isEqualToString:@"{ \"error\":\"No User!\"}"]) {
                ...
                NSArray *shots = jsonObjects;//[jsonObjects objectForKey:@"shots"];
...
- (void) startLoadingList{
    ...
    */
    ...
    if (isNewUpdate) {
        url = [NSString stringWithFormat:@"http://shots-city-api-staging.herokuapp.com/v1/shots/%@/comments?auth_token=%@&following=true&after=%@"
               , [self.shotData objectForKey:@"id"]//shot id
               , [[NSUserDefaults standardUserDefaults] objectForKey:@"auth_token"]
               , lastItemId
               ];
        ...
        url = [NSString stringWithFormat:@"http://shots-city-api-staging.herokuapp.com/v1/shots/%@/comments?auth_token=%@&following=true&before=%@"
               , [self.shotData objectForKey:@"id"]//shot id
               , [[NSUserDefaults standardUserDefaults] objectForKey:@"auth_token"]
               , lastItemId
               ];
...
- (IBAction)toggleAddComment:(id)sender{
    ...
    [self.parentViewController presentViewController:NC animated:NO
                                          completion:nil];
}
