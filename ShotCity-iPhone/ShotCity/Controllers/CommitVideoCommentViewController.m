//
//  CommitVideoCommentViewController.m
//  ShotsCity
//
...
@interface CommitVideoCommentViewController ()

@end

@implementation CommitVideoCommentViewController
...
- (void)viewDidLoad
{
    ...
    searchUserListViewController.view.frame = CGRectMake(0, 130, 320, self.view.frame.size.height - 130 - 220);
    ...
    [self captureThumbImage];
}

- (void)captureThumbImage{
    ...
    [moviePlayerController.view setFrame:theimageView.bounds];  // player's frame must match parent's
    ...
    [myImageView setImage:image320];
    
    [(AppDelegate*)[[UIApplication sharedApplication] delegate] setTempImage:myImageView.image];
    ...
    if(!data) {
        ...
        if (jsonData) {
            ...
            if (![aStr isEqualToString:@"{ \"error\":\"No User!\"}"]) {
                ...
                NSDictionary *user = [jsonObjects objectForKey:@"video_url"];
...
- (void) uploadData1{
    ...
    NSString *urlString = [NSString stringWithFormat:@"http://shots-city-api-staging.herokuapp.com/v1/shots?auth_token=%@"
                           , [[NSUserDefaults standardUserDefaults] objectForKey:@"auth_token"]];
	*/
    NSString *urlString = [NSString stringWithFormat:@"http://shots-city-api-staging.herokuapp.com/v1/shots/%@/comments?auth_token=%@"
                     , [[AppSharedData sharedInstance] selectedShotId]//shot id
                     , [[NSUserDefaults standardUserDefaults] objectForKey:@"auth_token"]];
...
/*    [parameters setValue:isFb == TRUE ? @"true" : @"false" forKey:@"facebook_share"];
    ...
    */
    ...
    NSString *FileParamConstant = @"video";
    
    
    //NSData *imageData = [[NSData alloc] initWithContentsOfFile:imagePath];
    NSData *imageData = nil;
    //imageData = [NSData dataWithContentsOfURL:[[AppSharedData sharedInstance] capturedMovieUrl]];
    
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *filePath = [[[AppSharedData sharedInstance] capturedMovieUrl] path];
    ...
    if (imageData) {
        [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
        [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"; filename=\"shot.mov\"\r\n", FileParamConstant] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    //NSData *imageData = [[NSData alloc] initWithContentsOfFile:imagePath];
    NSData *timageData = UIImageJPEGRepresentation(myImageView.image, .60);
	
    while ([timageData length] > maxFileSize && compression > maxCompression)
    {
        compression -= 0.1;
        timageData = UIImageJPEGRepresentation(myImageView.image, compression);
    }
    ...
    if (timageData) {
        ...
        [body appendData:[[NSString stringWithString:@"Content-Type: image/jpeg\r\n\r\n"] dataUsingEncoding:NSUTF8StringEncoding]];
        [body appendData:timageData];
        [body appendData:[[NSString stringWithFormat:@"\r\n"] dataUsingEncoding:NSUTF8StringEncoding]];
    }
    
    
    //audio
    if ([[AppSharedData sharedInstance] selectedSoundUrl] != nil) {
        NSData *soundData = nil;
        soundData = [NSData dataWithContentsOfURL:[[AppSharedData sharedInstance] selectedSoundUrl]];
        
        if (imageData) {
            FileParamConstant = @"sound";
